...
class Hannah {
    ...
    constructor() {
        ...
        left = Servo(ioexp, 10, hardware.pin5);
        right = Servo(ioexp, 10, hardware.pin7);
        right.setenabled(false);
        left.setenabled(false);
        local left_min = 0.035
        local right_min = 0.02
        local left_max = 0.13
        local right_max = 0.12
        left.scale(left_min,left_max);
        right.scale(right_min,right_max);
...
function fromdeg(angle){
  local srv_mult = (1.0 / 180.0);
  local result = (angle.tofloat() * srv_mult);
  return result;
}
function step(i){
  while (i > 0){
    hannah.right.write(fromdeg(40));
    hannah.left.write(fromdeg(155));
    imp.sleep(1);
    hannah.right.write(fromdeg(70));
    hannah.left.write(fromdeg(155));
    imp.sleep(1);
    hannah.right.write(fromdeg(0));
    hannah.left.write(fromdeg(125));
    imp.sleep(1);
    hannah.right.write(fromdeg(10));
    hannah.left.write(fromdeg(180));
    imp.sleep(1);
    i--
  }
}
hannah <- Hannah();
steps <- 0
hannah.left.setenabled(true);
hannah.right.setenabled(true);
hannah.right.write(fromdeg(40));
hannah.left.write(fromdeg(155));

step(7);

hannah.right.write(fromdeg(180));
hannah.left.write(fromdeg(0));
imp.sleep(2);
hannah.left.setenabled(false);
hannah.right.setenabled(false);
steps = 0;
