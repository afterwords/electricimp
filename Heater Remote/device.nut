...
agent.send("update", currentData);

hannah.on_temp_changed = function(state) {
  currentData.temp = hannah.temp.read();
  agent.send("update", currentData);
}
hannah.on_pot_changed = function(state) {
  currentData.pot = hannah.pot.read();
  agent.send("update", currentData);
}
hannah.on_btn1_changed = function(state) {
  if (state) {
    if (currentData.relay1){
      currentData.relay1 = 0;
      agent.send("update", currentData);
    } else {
      currentData.relay1 = 1;
      agent.send("update", currentData);
    }
  }
}

agent.on("updateRemote", function(data) {
  server.log(data.relay1);
  currentData.relay1 = data.relay1;
  if (currentData.relay1){
